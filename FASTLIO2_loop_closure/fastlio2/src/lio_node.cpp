...
struct NodeConfig
{
    ...
    int lidar_type = 0;   // 0 = Livox CustomMsg, 1 = PointCloud2
};
struct StateData
{
...
public:
    LIONode() : Node("lio_node")
    {
        ...
        m_imu_sub = this->create_subscription<sensor_msgs::msg::Imu>(m_node_config.imu_topic, rclcpp::SensorDataQoS(), std::bind(&LIONode::imuCB, this, std::placeholders::_1));

        if (m_node_config.lidar_type == 0) {
            // Livox CustomMsg
            m_lidar_sub = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(m_node_config.lidar_topic, 10, std::bind(&LIONode::lidarCBLivox, this, std::placeholders::_1));
        } else {
            // Standard PointCloud2
            auto qos = rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_sensor_data));
            m_lidar_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(m_node_config.lidar_topic, rclcpp::SensorDataQoS(), std::bind(&LIONode::lidarCBPCL, this, std::placeholders::_1));
        }
    ...
    void loadParameters()
    {
        ...
        if (config["lidar_type"]) {
            ...
            RCLCPP_INFO(this->get_logger(), "Using lidar_type = %d", m_node_config.lidar_type);
        }
    ...
    void lidarCBLivox(const livox_ros_driver2::msg::CustomMsg::SharedPtr msg) 
    {
    ...
    void lidarCBPCL(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        // 1) Convert ROS PointCloud2 to PCL XYZI
        ...
        // 2) Compute scan duration (approx.)
        double stamp = Utils::getSec(msg->header);  // use sensor timestamp
        static double last_cloud_stamp = -1.0;
        double dt = 0.1;  // default 0.1s (10Hz) if first scan
        if (last_cloud_stamp > 0.0) {
            dt = std::max(1e-3, stamp - last_cloud_stamp);
        }
        last_cloud_stamp = stamp;

        // 3) Copy points and fill curvature (used for cloud_end_time)
        ...
        for (int i = 0; i < N; ++i) {
            ...
            cloud->points[i].intensity = tmp->points[i].intensity;

            float alpha = (N > 1) ? static_cast<float>(i) / (N - 1) : 0.0f;
            cloud->points[i].curvature = std::max(0.001f, alpha * static_cast<float>(dt * 1000.0)); // in ms, avoid zero
        }

        // 4) Push into LIO buffer
        std::lock_guard<std::mutex> lock(m_state_data.lidar_mutex);
        m_state_data.lidar_buffer.emplace_back(stamp, cloud);
        m_state_data.last_lidar_time = stamp;

        // 5) Optional debug
        RCLCPP_INFO(this->get_logger(), "Received PCL scan with %d points, dt=%.3f ms", N, dt * 1000.0);
    }
    bool syncPackage()
    {
...
private:
    rclcpp::SubscriptionBase::SharedPtr m_lidar_sub;
