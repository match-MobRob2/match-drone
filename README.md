...
* **QGroundControl** 
* **teleop_twist_keyboard** package: Teleoperation ROS interface. For reference, see the [TELEOP](https://github.com/match-MobRob2/match-drone/wiki/teleoperation) WIKI documentation.
* **SLAM package**: FAST-LIO2 (Check this [SLAM selection](https://github.com/match-MobRob2/match-drone/wiki/slam-selection) WIKI documentation)
...
1. For installing **ROS2 jazzy**, **PX4**, **MAVROS**, **QGroundControl** ([Check connection establishment here](https://github.com/match-MobRob2/match-drone/blob/main/docs/QGroundControl.md)) and load custom models to PX4 run the following bash script or follow a step-by-step installation guide on the main branch of the repository:
   ```bash
   cd match-drone/setup/
   chmod +x setup.sh 
   ./setup.sh  
   ...
   - If gazebo harmonic (https://gazebosim.org/docs/harmonic/install_ubuntu/) was not installed for some reason, run the following:
      ```bash
...
2. Build and source slam workspace
      ```bash
      cd ~/slam_ws/
...
3. Run fast-lio2 package using custom YAML params file to start mapping (in new terminal)
   ...
   ```
      - See the documentation for [FAST-LIO2 custom params](https://github.com/match-MobRob2/match-drone/wiki/fast-lio2-params) under the repo's WIKI.
...
6. To save recorded map manually (in new terminal)
   ...
   ```
      - To see the documentation on [saving recorded map](https://github.com/match-MobRob2/match-drone/wiki/fast-lio2-params), please refer to the corresponding section in the repo's WIKI.
